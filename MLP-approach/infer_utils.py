...
def get_features(dataset, model, device, args_infer, test_idx):
    ...
    return img, mask, grasp, features

def test_single_point(grasp, mask, device, features, model, args_infer, point_idx):
    ...
    return preds.squeeze(), dif_n.squeeze(), single_point, single_point_gt